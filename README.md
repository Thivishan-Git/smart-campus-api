...
Conceptual Answers
Part 1.2 - JAX-RS Lifecycle.

The default is that JAX-RS will tend to allocate a new resource instance per request, rather than have one singleton resource serving all clients. This proves to be safer since specific user-request information is not disseminated to other users. Nevertheless, in this coursework the in-memory collections are kept in a shared data store meaning that the application still maintains a shared state. Concurrency has to be taken into account due to that. To prevent inconsistent updates, thread-safe collections or synchronization mechanisms would be significant in a larger system.
